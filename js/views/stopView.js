...
$(function(){
   app.StopView = Backbone.View.extend({
		...
		initialize: function() {
			...
			this.listenTo(app.App.ContentView, 'forceRefresh', this.updateArrivalList);

			this.updateArrivalList();
			app.App.Intervals.push(setInterval($.proxy(this.updateArrivalList, this), 60000));
		},
		...
		addArrival: function(arrival) {
			var arrivalView = new app.ArrivalView({model: arrival, stop: this.model});
