...
$(function(){
    app.ArrivalView = Backbone.View.extend({
		...
		initialize: function() {
			...
			app.App.Intervals.push(setInterval($.proxy(this.updateTime, this), 10000));
		},
		...
		showMap: function(scroll) {
			...
			if(!mapShowing) {
				...
				app.App.MapView.createStopMarker(this.options.stop);
    			app.App.MapView.createBusMarker(this.model);
    			this.resize(true);
		...
		resize: function(force) {
		    ...
		    if(force || mapShowing) {
				...
				app.App.MapView.$el.height(mapHeight);
				app.App.MapView.resize();
				app.App.MapView.setBounds();
	        }
		}
